...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const newPrenotazioni = Object.entries(res.sale["SAGRA"]).reduce(
          (acc, pixel) => {
  ...
  const deleteprenotazioni = async (value) => {
    function removePrenotazione(pixel, prenotazione) {
      const index = pixel.prenotazioni.findIndex(
        (p) =>
          p.data === prenotazione.data &&
          p.orario === prenotazione.orario &&
          p.user === prenotazione.user
      );
      return {
        ...pixel,
        prenotazioni: [
          ...pixel.prenotazioni.slice(0, index),
          ...pixel.prenotazioni.slice(index + 1),
        ],
      };
    }
    ...
    try {
      const res = await updateUserDocument(
        { uid: SALEUID },
        { sale: { SAGRA: newData } }
      );
      setDeletes(!deletes);
      ...
      urlencoded.append(
        "Body",
        `PRENOTAZIONE CANCELLATA, alla prossima !`
      );
  ...
  const sumMenu = (value) => {
    ...
    for (let i = 0; i < value.length; i++) {
      if (value[state].menu.toString() !== value[i].menu.toString()) {
        for (let a = 0; a < 4; a++) {
          menu[a] = value[state].menu[a] + value[i].menu[a];
        }
        state = i;
      }
    }
    ...
    for (let i = 0; i < value.length; i++) {
      if (
        value[i].tavolo !== temp &&
        !tavoli.find((elemento) => elemento === value[i].tavolo)
      ) {
  ...
  if (page.qr) return <Qr page={page} setPage={setPage} user={user} />;
  if (!page.state && Object.keys(onlydefault).length > 0)
    return (
      <div>
        <ToastContainer
          position="top-right"
          ...
          <Scroll>
            ...
            {Object.entries(onlydefault).map(([key, value], i) =>
              value.length > 0 ? (
                <Card key={i}>
                  <Svg
                    onClick={() => deleteprenotazioni(prenotazioni[key])}
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-6 w-6"
                    fill="none"
                    viewBox="0 0 24 24"
                  ...
                  <Right onClick={() => sumMenu(value)}>
                    <Testo line={"2vh"} padding={"10px"}>
                      Prenotazione confermata per il
                    </Testo>
                    <TestoBig line={"10vh"} size={"20vw"} padding={"10px"}>
                      {key.substr(0, 2)}
                    </TestoBig>
                    <Testo line={"5vh"} padding={"10px"}>
                      {key.substr(2)}{" "}
                    </Testo>
                  </Right>
                  <Left onClick={() => sumMenu(value)}>
                    <TestoBig line={"12vh"} size={"20vw"} padding={"20px"}>
                      {value.length}
                    </TestoBig>
                    <Testo line={"5vh"} padding={"10px"}>
                      POSTI
                ...
                <Eliminazione>
                  <div>
                    ...
                    <p>
                      Prenotazione in fase di cancellazione , clicca nuovamente
                      🗑 per confermare
                    </p>
                  </div>
                </Eliminazione>
    ...
    return (
      <div>
        ...
        <Title size={3}>
          Non hai prenotazioni al momento 😕 , CLICCA sul tasto + per iniziare
          😏
        </Title>
        <Link to="/data">
          <Svg1
    ...
    return (
      <Blocco>
        ...
        {page.data.find((elemento) => elemento.allergie !== "null") ? (
          <Allergie>
            <P1 color={"white"} size={"10px"}>
              Questa è una prenotazione con{" "}
            </P1>
            <img src={allergie}></img>
            <P2 color={"#ee404c"} size={"20px"}>
              ALLERGIE
            </P2>
          </Allergie>
        ) : (
