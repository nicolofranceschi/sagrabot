...
export default function Add() {
        ...
        .onSnapshot((doc) => {
          if (doc.data() !== null) {
            if (page.state === "pending") {
              const entrata = Object.entries(doc.data()).reduce(
                (acc, [chiave, valore]) => {
              ...
              const cassa = Object.entries(doc.data()).reduce(
                (acc, [chiave, valore]) => {
  ...
  const entrata = async () => {
    ...
    await updatedatasala({
      ...{ [confirm.value.key]: { ...confirm.value.value, state: "entrata" } },
      ...response,
    });

    const response = await getdatacoda();

    const updateddata = Object.entries(response).reduce(
      (acc, [chiave, valore]) => {
        if (chiave === confirm.value.key)
          return {
            ...acc,
            [confirm.value.key]: { ...valore, state: "entrato" },
          };
  ...
  const onUpdate = async (data, e) => {
    ...
    const updateddata = Object.entries(response).reduce(
      (acc, [chiave, valore]) => {
        if (chiave === page.value.key) {
          user.numero = valore.user;
          user.name = valore.nome;
          return {
            ...acc,
            [page.value.key]: {
              ...valore,
              data: new Date(),
              Ntavoli: data.tavolo.split(" "),
              state: "complete",
            },
          };
        } else return { ...acc, [chiave]: valore };
    ...
    try {
      ...
      urlencoded.append("To", user.numero);
      urlencoded.append(
        "MessagingServiceSid",
      ...
      urlencoded.append(
        "Body",
        `Ciao ${user.name}, sei stato assegnato al tavolo ${data.tavolo}, ti aspettiamo all entrata !`
      );
  ...
  return (
    <>
      <Bar>
        <Statebutton
          color={
            page.state === "pending" ? "var(--line)" : "var(--black-light)"
          }
          onClick={() => setPage({ state: "pending" })}
        >
        ...
        <Statebutton
          color={
            page.state === "complete" ? "var(--line)" : "var(--black-light)"
          }
          onClick={() => setPage({ state: "complete" })}
        >
        ...
        {page.state === "pending" ? (
          <>
            <Containerscr>
              <Scroll>
                {data
                  ? Object.entries(data)
                      .sort((a, b) => b[1].data.seconds - a[1].data.seconds)
                      .reverse()
                      .map(([key, value]) => (
                        <Card
                          onClick={() =>
          ...
          <Container>
            <LoginForm autocomplete="off" onSubmit={handleSubmit(onUpdate)}>
              <>
                <Label>
                  <Span font={"0.7rem"} bold={"normal"}>
                    1 -
                  </Span>
                  <Span font={"1.5rem"} bold={"normal"}>
                    Inserisci il{" "}
                  </Span>
                  <Span font={"1.5rem"} bold={"bold"}>
                    tavolo
                  </Span>
                </Label>
                ...
                <Input
                  ...
                  {...register("tavolo")}
                />
                <ButtonBig type="submit" margin="5vh 0 0 0" padding="15px 0">
                  AGGIUNGI
                </ButtonBig>
              </>
            </LoginForm>
                  ...
                  ? Object.entries(data)
                      ...
                      .map(([key, value]) => (
                        <Card
                          key={key}
                          onClick={() =>
                            setConfirm({ state: 1, value: { key, value } })
                          }
                        >
                          <Right>
                            <Testo
          ...
          <Accordion show={step === 4}>
            <>
              ...
              <Select {...register("data")}>
                <option value="10 Ottobre-alle 12:00">
                  10 Ottobre-alle 12:00
                </option>
                <option value="17 Ottobre-alle 12:00">
                  17 Ottobre-alle 12:00
                </option>
                <option value="24 Ottobre-alle 12:00">
                  24 Ottobre-alle 12:00
                </option>
                <option value="31 Ottobre-alle 12:00">
                  31 Ottobre-alle 12:00
                </option>
              </Select>
              <Button
                onClick={next}
