...
export default function Bussola ({page}) {
  ...
  return <MappaPrenotazioni data={data} page={page} />
}

function MappaPrenotazioni ({ data ,page }) {
    ...
    <>
      <Container ref={DrawingGrid}>
        <Grid gridSize={gridSize} pixelSize={pixelSize} tabIndex={0} ref={pixelsContainerRef}>
          <Pixels data={data} selected={selected} page={page} setSelected={setSelected} orario={orario}  />
        </Grid>
      </Container>
    </>
  );
...
const Pixels = memo(({ data, selected, setSelected, orario, onSelect,page }) => (
  <>
  {cells.map((_, i) => (
    <ObservedPixel key={i}>
      {ref => <Pixel i={i} data={data[i]} selected={selected[i]} posti={page.value} setSelected={setSelected} orario={orario} onSelect={onSelect} ref={ref} />}
    </ObservedPixel>
  ))}
  </>
));
