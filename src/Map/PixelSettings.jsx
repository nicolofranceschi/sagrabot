...
export const Framerdiv = styled(motion.div)`
  display: flex;
  flex-direction: column;
...
export default function PixelSettings({ datistatistici, setOrario, open , setOpen, setFluo}) {
  const { height, width } = useWindowSize();
  const { register, handleSubmit } = useForm();
  const onSubmit = (data) => {
    console.log({
      data: data.data.substr(0, 10),
      orario: data.data.substr(11),
    });
  ...
  return (
    <>
      ...
      <Dati>
        <Datip>
          <Fumetto content="Livello ottimizzazione sala" direction="right">
            <p>
              {Math.round(datistatistici.posti / datistatistici.posticovid)}
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
                fill="none"
      ...
      >
        <p>
          {open.data.nome} {open.data.cognome}
        </p>
        <span>Posti: {open.data.posti}</span>
        <span>{open.data.numero}</span>
        <Svgclose
         onClick={()=>{
